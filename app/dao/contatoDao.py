...
class ContatoDao():
    ...
    def saveContato(self,cpf,contato:ContatoModel):
        ...
        cursor.execute('insert into contatos (tipo,ccontato,cdescricao, pcpf) values(%s,%s,%s,%s)',(contato.tipo,contato.contato,contato.descricao,cpf))
        mysql.connection.commit()
        cursor.close()

    def getContatoContatosPorCpf(self,cpf):
        cursor = mysql.connection.cursor()
        cursor.execute('select tipo, ccontato, cdescricao from contatos where pcpf = %s',(cpf,))
    ...
    def updateContato(self,cpf,contato:ContatoModel):
        cursor = mysql.connection.cursor()
        cursor.execute('update contatos set tipo = %s, ccontato = %s, cdescricao = %s where pcpf = %s and ccontato = contato.contato',(contato.tipo,contato.contato,contato.descricao,cpf))
        mysql.connection.commit()
        cursor.close()