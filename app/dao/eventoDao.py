...
class EventoDao():
    ...
    def setEvento(self, id_local, evento: EventoModel):
        ...
        cursor.execute('insert into eventos(comeco, fim, duracao, quantidade, descricao, lid_local) values(%s,%s,%s,%s,%s,%s)',
                       (evento.comeco, evento.fim, evento.duracao, evento.quantidade, evento.descricao, evento.local.id))
    ...
    def getEventosPorLocal(self, id_local):
        cursor = mysql.connection.cursor()
        cursor.execute(
            'select * from eventos where lid_local = %s', (id_local))
        ...
        if result:
            for i in result:
                ...
                evento = eventoModel(
                    id, comeco, fim, duracao, quantidade, descricao, local)
    ...
    def updateEventos(self, evento: EventoModel):
        cursor = mysql.connection.cursor()
        cursor.execute('update eventos set comeco = %s, fim = %s, duracao = %s, quantidade = %s, descricao = %s, id_local = %s where id_evento = %s',
                       (evento.comeco, evento.fim, evento.duracao, evento.quantidade, evento.descricao, evento.local.id))
        mysql.connection.commit()
        cursor.close()
