...
class ParticipanteEventoDao():
    ...
    def setParticipanteEvento(self, participanteEvento:ParticipanteEventoModel):
        ...
        cursor.execute('insert into participa (ppcpf,eid_evento,horario,tratamento_especial) values(%s ,%s ,%s ,%s)',(participanteEvento.cpf, participanteEvento.eventoId, participanteEvento.horario,participanteEvento.tratamentoEsp))
        mysql.connection.commit()
        cursor.close()

    def getParticipanteEvento(self,id,cpf):
        cursor = mysql.connection.cursor()
        cursor.execute('select * from participa where ppcpf = %s and eid_evento' ,(id,cpf))
    ...
    def deleteParticipanteEvento(self,id,cpf):
        cursor = mysql.connection.cursor()
        cursor.execute('delete from participa where ppcpf = %s and eid_evento', (cpf,id))
        mysql.connection.commit()
        cursor.close()
